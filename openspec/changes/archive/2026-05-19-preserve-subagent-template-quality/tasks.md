...
- [x] 1.1 Extract the source prompt obligations for `designer`,
  `orchestrator-explorer`, `fixer`, `librarian`, `observer`, and `oracle` from
  `references/oh-my-opencode-slim/src/agents/*.ts`.
- [x] 1.2 Define curated coverage points for each bundled template covering
  role, behavior, constraints, output expectations, and specialized guidance.
- [x] 1.3 Define required Codex adaptation notes for OpenCode-specific tools,
  MCP names, permissions, or runtime assumptions that cannot be preserved
  directly.

## 2. Template Updates

- [x] 2.1 Update `orchestrator-explorer.toml` so it preserves the source
...
- [x] 2.2 Update `fixer.toml` so it preserves the source fixer prompt's
...
- [x] 2.3 Update `librarian.toml` so it preserves the source librarian prompt's
...
- [x] 2.4 Update `oracle.toml` so it preserves the source oracle prompt's
...
- [x] 2.5 Update `observer.toml` so it preserves the source observer prompt's
...
- [x] 2.6 Update `designer.toml` so it preserves the source designer prompt's
  ...
  Codex.
- [x] 2.7 Add or update adaptation comments/fixtures documenting intentional
  substitutions or omissions for each template.

## 3. Verification

- [x] 3.1 Add deterministic template quality tests that validate each bundled
  template against the curated coverage points and adaptation notes.
- [x] 3.2 Keep existing installer rendering tests passing for TOML discovery,
  model replacement, disabled-agent removal, and bundled agent output names.
- [x] 3.3 Update installer skill documentation to state that bundled templates
