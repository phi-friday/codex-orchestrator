...
- Migrate non-hook direct stdout and stderr writes to explicit console severity methods.
...
3. Non-hook direct stdout and stderr writes are treated as an anti-pattern for CLI messages.

   For human-facing script output, console severity methods communicate intent better and handle newline behavior consistently. Existing non-hook stream writes should be replaced with specific methods whether they use `process.stdout.write` / `process.stderr.write`, imported `stdout.write` / `stderr.write` calls, or fd-based helpers such as `writeSync(1, ...)` / `writeSync(2, ...)`: `console.info` for normal status and usage output, `console.error` for errors, `console.warn` for warnings, and `console.debug` for diagnostic detail.
